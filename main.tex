...
\section{ProMPs}
\subsection{Recap}
From \cite{paraschos_probabilistic_2013, paraschos_using_2018}:
\begin{itemize}
    ...
    \item $p(\bm{w};\bm{\theta})$: prior over the weight vector $\bm{w}$, with parameters $\bm{\theta}$, assumed to be Gaussian
\end{itemize}
...
\end{gather}
\Cref{eq:HBM} is illustrated in \Cref{fig:HBM}.
...
Using Bayes rule: $P(A|B) = \frac{p(B|A) \cdot P(A)}{P(B)}$
\begin{align}
