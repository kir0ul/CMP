...
\begin{align}
  \bm{\mu}_{\bm{y}_{t}}(\bm{y}_t^\star) &\stackrel{?}{=} \bm{\mu_{w}}^{[new]}(\bm{y}_t^\star)\\
  \bm{\Psi}_t^\top \bm{\mu_{w}}(\bm{y}_t^\star) &\stackrel{?}{=} \bm{\mu_w} + \bm{\Sigma_w}\bm{\Psi}_t \Big(\bm{\Sigma}_y^\star \bm{\Psi}_t^\top \bm{\Sigma_w}\bm{\Psi}_t \Big)^{-1} (\bm{y}_t^\star - \bm{\Psi}_t^\top \bm{\mu_w})
  \end{align}
\todo[inline]{ToDo}
...
\end{align}
\todo[inline]{Missing developments}
\end{proof}
