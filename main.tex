...
\Repeat{convergence}{
  ...
  \vspace{-1.5em}
      \begin{flalign*}%
        &\mathcal{L} \gets \sum^{N}_{n=1} \ln
          \begin{bmatrix}
          \sum^{K}_{k=1} \pi_{k}  \mathcal{N}(\bm{\mathrm{x}}_{n}|\bm{\mu}_{k}, \bm{\Sigma}_{k})
          \end{bmatrix}&
      \end{flalign*}
}
...
\begin{table}
  ...
  \begin{tabular}{ll}
    ...
    $\bm{\mu}_{kt}$ & mean of $t_{i}$ for the $k^{\text{th}}$ Gaussian cluster\\
     $h_{k}(t_{i})$ & responsibility of the $k^{\text{th}}$ mixture component at timestep $t_{i}$\\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{gather}
  ...
  \begin{bmatrix}
    \cdot & \cdot\\[0.5em]
    \cdot & \bm{\Psi}^\top_{t} \bm{\Sigma_w} \bm{\Psi}_t + \bm{\Sigma}_y
  \end{bmatrix}
  }
\end{gather}
...
\begin{proof}[Proof of \Cref{eq:mu-cond-new} and \Cref{eq:sigma-cond-new}]\label{proof:approx-multimod-to-single}
